...
username = os.environ.get('bubimwby') # PROXY_USER
password = os.environ.get('9410uu0vec9d') # PROXY_PASS
product_title = ""
reviewList = []
...
async def extractReviews(reviewURL, pageNumber):
  ...
  for item in reviews:
    ...
    #   f.write(str(item))
    review = {
      'productTitle': soup.title.text.replace("Amazon.com:Customer reviews: ", "").strip(),
...
async def getTotalPages(url):
  ...
  product_title = soup.title.text.replace("Amazon.com:Customer reviews: ", "").strip()[0:16].replace(' ', '').replace(',', '')
...
async def extractProfile(url):
  ...
  product_title = soup.title.text.replace("Amazon.com:Customer reviews: ", "").strip()[0:64]
